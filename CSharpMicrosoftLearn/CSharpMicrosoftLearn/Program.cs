...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            // Display all dogs with multiple search characteristics + improved search animation (spinning + countdown)

            string dogCharacteristicsInput = "";

            // Ask for comma-separated search terms (must not be empty)
            while (string.IsNullOrWhiteSpace(dogCharacteristicsInput))
            {
                Console.WriteLine("\r\nEnter dog characteristics to search for separated by commas");
                readResult = Console.ReadLine();

                if (readResult != null)
                    dogCharacteristicsInput = readResult.Trim();
            }

            // Split into terms, trim entries, lowercase, remove empties
            string[] dogSearchTerms = dogCharacteristicsInput
                .ToLower()
                .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

            // If user typed only commas/spaces, re-prompt
            while (dogSearchTerms.Length == 0)
            {
                ...
                dogCharacteristicsInput = readResult?.Trim() ?? "";
                dogSearchTerms = dogCharacteristicsInput
                    .ToLower()
                    .Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
            }
            ...
            // Improved "spinning" icons
            string[] searchingIcons = { "/", "-", "\\", "|" };

            // Loop through animals, only dogs
            for (int i = 0; i < maxPets; i++)
            {
                if (!ourAnimals[i, 1].Contains("dog"))
                    continue;

                // Combine searchable text (physical + personality)
                string dogDescription = (ourAnimals[i, 4] + "\r\n" + ourAnimals[i, 5]).ToLower();

                bool thisDogMatched = false;

                // Search each term for this dog
                foreach (string term in dogSearchTerms)
                {
                    // Improved animation with countdown 2..0
                ...
                // If this dog matched at least one term, display details once
                if (thisDogMatched)
                {
